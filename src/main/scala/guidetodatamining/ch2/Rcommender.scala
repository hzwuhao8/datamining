...
object Recommender extends util.Log {
...
class Recommender(data: Recommender.UserMap, k: Int = 1, metric: Recommender.MetricFunc= Recommender.pearsonMetric, n: Int = 5) {
  def recommend(username: String): Seq[(String, Double)] = Recommender.recommed(username, data, metric, k, n)
}